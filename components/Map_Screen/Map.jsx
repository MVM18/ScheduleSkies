...
function MapResizeHelper() {
  ...
  useEffect(() => {
    ...
    return () => { window.clearTimeout(t); window.removeEventListener('resize', fix); };
  }, [map]);
  return null;
}
...
function MapPickClickHandler({ enabled, onLatLngClick }) {
  useMapEvents({
    click(e) { if (enabled) onLatLngClick(e.latlng); },
  });
  return null;
}
...
function MapClickHandler({ mode, onSetOrigin, onSetDestination, isLocked }) {
  useMapEvents({
    click: async (e) => {
      if (isLocked) return;
      if (mode === 'origin') onSetOrigin(e.latlng);
      else if (mode === 'destination') onSetDestination(e.latlng);
    },
  });
  return null;
}
...
function SearchInput({ placeholder, value, onChange, onSelect, icon, isMobile, darkMode = false }) {
            ...
            <div
              ...
              style={{
                ...
                borderBottom: i < suggestions.length - 1
                  ? `1px solid ${darkMode ? '#334155' : '#f0f0f0'}`
                  : 'none',
                color: darkMode ? '#cbd5e1' : '#333',
                display: 'flex', alignItems: 'center', gap: '8px',
              }}
              onMouseEnter={e => (e.currentTarget.style.background = darkMode ? '#1e3a5f' : '#EBF4FF')}
              onMouseLeave={e => (e.currentTarget.style.background = darkMode ? '#1e293b' : 'white')}
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#718096" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
...
const getRouteSegmentsWithTraffic = (routeCoords) => {
  ...
  for (let i = 0; i < totalSegments; i++) {
    ...
    segments.push({
      start: routeCoords[i], end: routeCoords[i + 1],
      color: traffic.color, level: traffic.level, delayFactor: traffic.delayFactor,
    });
  }
  return segments;
};
...
const getTrafficSummary = (segments) => {
  ...
  if (heavyCount > segments.length * 0.3) { overallLevel = 'Heavy'; overallColor = '#C62828'; }
  else if (moderateCount > segments.length * 0.4) { overallLevel = 'Moderate'; overallColor = '#F57F17'; }
...
const findPlacesAlongRoute = async (routeCoords) => {
    ...
    for (const point of samplePoints) {
      ...
      if (data.features && data.features.length > 0) {
        data.features.forEach(feature => {
          ...
          seenNames.add(name);
          places.push({ name, lat, lng, type: placeType, address: feature.properties.address_line1 || '', distance: Math.round(distance), rating: feature.properties.rating || null });
        });
      }
    }
  } catch (error) {
  ...
  if (places.length === 0) {
    ...
    mockPlaces.forEach(place => {
      if (!seenNames.has(place.name)) { seenNames.add(place.name); places.push(place); }
    });
  }
...
const findPlacesNearPoint = async (lat, lng) => {
  try {
    if (!GEOAPIFY_API_KEY) { console.error('GEOAPIFY_API_KEY is not set'); return []; }
    const categories = ['catering.restaurant','catering.cafe','parking','tourism.attraction','leisure.park','commercial.shopping_mall','accommodation.hotel'];
    ...
    const response = await fetch(url);
    const data = await response.json();
    if (data.statusCode || data.message) { console.error('Geoapify API error:', data.statusCode, data.message); return []; }
    if (!data.features || data.features.length === 0) { console.warn('Geoapify returned no features for:', lat, lng); return []; }
    const places = [];
    const seenNames = new Set();
    data.features.forEach(feature => {
      ...
      seenNames.add(name);
      places.push({ name, lat: lat2, lng: lng2, type: placeType, address: feature.properties.address_line1 || '', distance: Math.round(distance), rating: feature.properties.rating || null });
    });
    places.sort((a, b) => a.distance - b.distance);
    return places.slice(0, 25);
  } catch (error) {
...
const MapScreen = ({
  ...
  useEffect(() => {
    const check = () => setDarkMode(document.body.getAttribute('data-theme') === 'dark');
    check();
  ...
  useEffect(() => {
    if (itineraryWaypoints?.length) {
      ...
      if (first) {
        ...
        setDestination(dest); setDestText(dest.label); setFlyTo([dest.lat, dest.lng]);
      }
    }
  }, [itineraryWaypoints]);
  ...
  const confirmPick = () => {
    ...
    if (onPickConfirm) { onPickConfirm(pickResult); return; }
    try {
      const storageKey = `scheduleSkies_mapPick_${pickContext}`;
      sessionStorage.setItem(storageKey, JSON.stringify({ context: pickContext, ...pickResult, ts: Date.now() }));
    } catch (e) { console.error(e); }
    router.push(returnPath);
  };

  const cancelPick = () => {
    if (onPickCancel) { onPickCancel(); return; }
    router.push(returnPath);
  };
  ...
  const goToNextWaypoint = () => {
    ...
    if (nextIdx < waypointsList.length) {
      ...
      if (wp.lat && wp.lng) {
        ...
        setDestination(dest); setDestText(dest.label); setFlyTo([dest.lat, dest.lng]);
        if (destination) { setOrigin(destination); setOriginText(destination.label); }
      }
    }
  };
  ...
  const getUserLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      position => {
        ...
        if (pickMode) { handlePickMapClick({ lat, lng }); return; }
        const userLoc = { lat, lng, label: 'Your Location' };
        setUserLocation(userLoc); setOrigin(userLoc); setOriginText('Your Location');
        setFlyTo([lat, lng]); setWeatherLocation(userLoc);
      },
      error => {
  ...
  const handleMapSetOrigin = async latlng => {
    ...
    setOrigin(newOrigin); setUserLocation(newOrigin); setOriginText(newOrigin.label);
    setFlyTo([newOrigin.lat, newOrigin.lng]); setWeatherLocation(newOrigin);
    setRouteCoords(null); setRouteSegments([]); setRouteInfo(null); setRouteError(''); setTrafficSummary(null);
  ...
  const handleMapSetDestination = async latlng => {
    ...
    setDestination(newDest); setDestText(newDest.label); setFlyTo([newDest.lat, newDest.lng]);
    setRouteCoords(null); setRouteSegments([]); setRouteInfo(null); setRouteError('');
    setTrafficSummary(null); setIsRerouted(false); setReroutedPlace(null);
  ...
  const fetchPlacesAlongRoute = async coords => {
    ...
    setNearbyPlaces(places); setIsLoadingPlaces(false);
  };
  ...
  const handleOriginSelect = place => {
    setOrigin(place); setUserLocation(place); setFlyTo([place.lat, place.lng]);
    setRouteCoords(null); setRouteSegments([]); setRouteInfo(null);
    setRouteError(''); setTrafficSummary(null); setIsRerouted(false); setReroutedPlace(null);
  };

  const handleDestSelect = place => {
    setDestination(place); setFlyTo([place.lat, place.lng]);
    setRouteCoords(null); setRouteSegments([]); setRouteInfo(null);
    setRouteError(''); setTrafficSummary(null); setIsRerouted(false); setReroutedPlace(null);
  };
  ...
  const fetchRoute = async (org, dst, mode, isReroute = false) => {
    ...
    setIsRouting(true); setRouteError(''); setRouteCoords(null);
    setRouteSegments([]); setRouteInfo(null); setTrafficSummary(null);
    try {
      ...
      setRouteInfo({ distance: (feature.properties.distance / 1000).toFixed(1), time: baseTime, adjustedTime, trafficDelay: adjustedTime - baseTime });
      if (coords.length) {
  ...
  const handleRerouteToPlace = async place => {
    if (!origin && !userLocation) { alert('Please set an origin first (use "My Location" or enter a starting point)'); return; }
    const currentOrigin = origin || userLocation;
    if (!isRerouted && destination) setOriginalDestination(destination);
    setDestination(place); setDestText(place.name); setReroutedPlace(place); setIsRerouted(true);
  ...
  const undoReroute = async () => {
    if (originalDestination) {
      const currentOrigin = origin || userLocation;
      setDestination(originalDestination); setDestText(originalDestination.label);
      setIsRerouted(false); setReroutedPlace(null);
  ...
  const clearAll = () => {
    ...
    setRouteCoords(null); setRouteSegments([]); setRouteInfo(null); setRouteError('');
    setNearbyPlaces([]); setTrafficSummary(null); setIsRerouted(false);
    setReroutedPlace(null); setOriginalDestination(null); setFlyTo(defaultPosition);
  };
  ...
  useEffect(() => { setWeatherLocation({ lat: 10.3204, lng: 123.9242 }); }, []);
  ...
  const mapContent = (
    <>
      ...
      <TileLayer
        url={darkMode ? 
          `https://maps.geoapify.com/v1/tile/positron/{z}/{x}/{y}.png?apiKey=${GEOAPIFY_API_KEY}` :
      ...
      {showPlacesPanel && (
        <>
          <motion.div
            ...
            style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 1001, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(2px)', pointerEvents: 'auto' }}
          />
          <motion.div
            key="sheet"
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'center', padding: '12px 0 8px' }}>
              <div style={{ width: '40px', height: '4px', background: dm('#cbd5e0', '#475569'), borderRadius: '4px' }} />
            </div>

            {/* Header */}
            <div style={{ background: 'linear-gradient(135deg, #1a365d 0%, #2b6cb0 100%)', margin: '0 16px 12px 16px', borderRadius: '20px', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div>
                  <div style={{ fontSize: '14px', fontWeight: '800', color: 'white', letterSpacing: '-0.3px' }}>Suggested Places</div>
                  <div style={{ fontSize: '10px', color: '#bee3f8', marginTop: '2px' }}>{nearbyPlaces.length} spots nearby</div>
                </div>
              </div>
              <button onClick={() => setShowPlacesPanel(false)} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', borderRadius: '30px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'white', fontSize: '16px' }}>✕</button>
            </div>

            {/* Filter chips */}
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', padding: '0 16px 12px 16px', borderBottom: `1px solid ${dm('#edf2f7', '#334155')}` }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: placeTypeFilter === option.key
                      ...
                      : dm('#f0f4f8', '#1e293b'),
                    color: placeTypeFilter === option.key ? 'white' : dm('#2d3748', '#cbd5e1'),
                    cursor: 'pointer', fontSize: '12px', fontWeight: '600',
                    display: 'inline-flex', alignItems: 'center', gap: '6px',
                    boxShadow: placeTypeFilter === option.key ? '0 2px 8px rgba(43,108,176,0.3)' : 'none',
                    transition: 'all 0.2s',
                  }}
                  onMouseEnter={e => { if (placeTypeFilter !== option.key) e.currentTarget.style.background = dm('#e2e8f0', '#334155'); }}
                  onMouseLeave={e => { if (placeTypeFilter !== option.key) e.currentTarget.style.background = dm('#f0f4f8', '#1e293b'); }}
                >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', padding: '16px' }}>
              {getFilteredPlaces().length === 0 ? (
                <div style={{ textAlign: 'center', fontSize: '13px', color: dm('#718096', '#64748b'), padding: '32px 16px', background: dm('#f7fafc', '#0f172a'), borderRadius: '20px' }}>
                ...
                getFilteredPlaces().map((place, i) => (
                  <motion.div
                    ...
                    style={{
                      ...
                      border: `1px solid ${dm('#edf2f7', '#334155')}`,
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', padding: '12px' }}>
                      <div style={{ width: '48px', height: '48px', background: `linear-gradient(135deg, ${getPlaceColor(place.type)}20, ${getPlaceColor(place.type)}40)`, borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: '14px' }}>
                        {getPlaceIcon(place.type, 24, getPlaceColor(place.type))}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                          <div style={{ flex: 1 }}>
                            <h4 style={{ fontSize: '14px', fontWeight: '700', color: dm('#1a202c', '#e2e8f0'), margin: 0 }}>{place.name}</h4>
                            {place.address && (
                              <p style={{ fontSize: '10px', color: dm('#718096', '#64748b'), margin: '4px 0 0', lineHeight: 1.3 }}>
                                {place.address.length > 45 ? `${place.address.substring(0, 45)}…` : place.address}
                              </p>
                            )}
                          </div>
                          {place.rating && (
                            <div style={{ background: '#fef5e7', padding: '3px 8px', borderRadius: '30px', fontSize: '11px', fontWeight: '600', color: '#d69e2e', display: 'flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}>
                              ⭐ {place.rating.toFixed(1)}
                            </div>
                          )}
                        </div>
                        <div style={{ display: 'flex', gap: '12px', marginTop: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                          <span style={{ fontSize: '10px', background: dm('#f0f4f8', '#0f172a'), padding: '3px 10px', borderRadius: '30px', color: dm('#2c5282', '#93c5fd'), fontWeight: '600', textTransform: 'capitalize' }}>
                            {place.type}
                          </span>
                          <span style={{ fontSize: '10px', color: dm('#4a5568', '#94a3b8'), display: 'flex', alignItems: 'center', gap: '3px' }}>
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={dm('#4a5568', '#94a3b8')} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                              <path d="M12 11m-3 0a3 3 0 1 0 6 0a3 3 0 1 0 -6 0"/>
                              <path d="M17.657 16.657l-4.243 4.243a2 2 0 0 1 -2.827 0l-4.244 -4.243a8 8 0 1 1 11.314 0z"/>
                            </svg>
                    ...
                    <div style={{ borderTop: `1px solid ${dm('#edf2f7', '#334155')}`, padding: '8px 12px', background: dm('#fafcff', '#0f172a'), display: 'flex', justifyContent: 'flex-end' }}>
                      <span style={{ fontSize: '11px', fontWeight: '600', color: '#ff9800', display: 'flex', alignItems: 'center', gap: '4px' }}>Reroute here →</span>
                    </div>
                  </motion.div>
                ))
              )}
            ...
            {isRerouted && reroutedPlace && (
              <div style={{ padding: '12px 16px 20px', borderTop: `1px solid ${dm('#edf2f7', '#334155')}`, background: dm('#fffaf0', '#1a1200') }}>
                <div style={{ background: dm('#fff3e0', '#1a1200'), padding: '10px 14px', borderRadius: '16px', border: `1px solid ${dm('#ffe0b5', '#92400e')}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
                  ...
                  <button onClick={undoReroute} style={{ padding: '6px 12px', background: '#ff9800', color: 'white', border: 'none', borderRadius: '30px', cursor: 'pointer', fontSize: '11px', fontWeight: '600' }}>Undo reroute</button>
                </div>
              </div>
            )}
          </motion.div>
    ...
    >
      ...
      {pickMode ? (
        <div style={{
          ...
          zIndex: 1002, pointerEvents: 'auto',
          background: dm('rgba(255,255,255,0.97)', 'rgba(15,23,42,0.97)'),
          borderRadius: '18px', padding: '14px 16px',
          boxShadow: '0 4px 18px rgba(0,0,0,0.13)',
        }}>
          <div style={{ fontSize: '13px', fontWeight: '800', color: dm('#1A365D', '#93c5fd'), marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={dm('#1A365D', '#93c5fd')} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M12 11m-3 0a3 3 0 1 0 6 0a3 3 0 1 0 -6 0"/>
              <path d="M17.657 16.657l-4.243 4.243a2 2 0 0 1 -2.827 0l-4.244 -4.243a8 8 0 1 1 11.314 0z"/>
            </svg>
            Pin your {isActivityPickContext(pickContext) ? 'activity' : 'event'} location
          </div>
          <p style={{ fontSize: '12px', color: dm('#4A5568', '#94a3b8'), margin: '0 0 10px', lineHeight: 1.45 }}>
            Tap anywhere on the map to place or move the pin.
            {pickHintLabel ? <span style={{ display: 'block', marginTop: '6px', fontSize: '11px', opacity: 0.85 }}>Hint: {pickHintLabel}</span> : null}
          </p>
          {pickLoading && <div style={{ fontSize: '11px', color: '#718096', marginBottom: '8px' }}>Resolving address…</div>}
          {pickedLabel && (
            <div style={{ fontSize: '11px', color: dm('#2C5282', '#93c5fd'), marginBottom: '10px', padding: '8px 10px', background: dm('#EBF8FF', '#1e3a5f'), borderRadius: '10px', maxHeight: '72px', overflowY: 'auto' }}>
              {pickedLabel}
            </div>
          )}
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button type="button" onClick={cancelPick} style={{ padding: '8px 16px', borderRadius: '10px', border: `1px solid ${dm('#CBD5E0', '#334155')}`, background: dm('white', '#1e293b'), color: dm('#4A5568', '#94a3b8'), fontWeight: 600, fontSize: '13px', cursor: 'pointer' }}>Cancel</button>
        ...
        <>
          ...
          {!pickMode && (
            <div style={{ position: 'absolute', top: '60px', left: '10px', right: '10px', zIndex: 1002, pointerEvents: 'auto', display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <button onClick={() => setShowSearchPanel(!showSearchPanel)} style={{ backgroundColor: showSearchPanel ? '#2C5282' : dm('#ffffff', '#1e293b'), color: showSearchPanel ? 'white' : 'lightblue', border: 'none', borderRadius: '30px', padding: '8px 16px', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', cursor: 'pointer' }}>
              ...
              <button onClick={() => setShowPlacesPanel(!showPlacesPanel)} style={{ backgroundColor: showPlacesPanel ? '#FF9800' : dm('#ffffff', '#1e293b'), color: showPlacesPanel ? 'white' : '#FF9800', border: 'none', borderRadius: '30px', padding: '8px 16px', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', cursor: 'pointer' }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M12 11m-3 0a3 3 0 1 0 6 0a3 3 0 1 0 -6 0"/>
                  <path d="M17.657 16.657l-4.243 4.243a2 2 0 0 1-2.827 0l-4.244-4.243a8 8 0 1 1 11.314 0z"/>
                </svg>
              ...
              {waypointsList.length > 0 && (
                <button onClick={() => setShowItineraryPanel(!showItineraryPanel)} style={{ backgroundColor: showItineraryPanel ? '#7B1FA2' : dm('#ffffff', '#1e293b'), color: showItineraryPanel ? 'white' : '#7B1FA2', border: 'none', borderRadius: '30px', padding: '8px 16px', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', cursor: 'pointer' }}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {showSearchPanel && (
              <motion.div
                ...
                style={{
                  ...
                  zIndex: 1002, pointerEvents: 'auto',
                  background: dm('rgba(255,255,255,0.98)', 'rgba(15,23,42,0.97)'),
                  backdropFilter: 'blur(10px)',
                  borderRadius: '28px',
                  boxShadow: dm('0 20px 35px -12px rgba(0,0,0,0.2), 0 0 0 1px rgba(0,0,0,0.02)', '0 20px 35px -12px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.04)'),
                  overflow: 'hidden', transformOrigin: 'top center',
                }}
              >
                {/* Header */}
                <div style={{ background: 'linear-gradient(135deg, #1a365d 0%, #2b6cb0 100%)', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '16px', fontWeight: '800', color: 'white', letterSpacing: '-0.3px' }}>Route Planner</div>
                      <div style={{ fontSize: '11px', color: '#bee3f8', marginTop: '2px' }}>Plan your journey</div>
                    </div>
                  </div>
                  <button
                    onClick={() => setShowSearchPanel(false)}
                    style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: '30px', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'white', fontSize: '18px', transition: 'background 0.2s' }}
                ...
                <div style={{ padding: '20px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                      <div style={{ flex: 1 }}>
                        <SearchInput placeholder="From – your starting point" value={originText} onChange={setOriginText} onSelect={handleOriginSelect} icon="🟦" isMobile={true} darkMode={darkMode} />
                      </div>
                      <motion.button
                        whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                        onClick={getUserLocation}
                        style={{ padding: '10px 12px', borderRadius: '16px', border: 'none', background: dm('#edf2f7', '#1e293b'), color: '#2c5282', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    ...
                    <SearchInput placeholder="To – your destination" value={destText} onChange={setDestText} onSelect={handleDestSelect} icon="🔴" isMobile={true} darkMode={darkMode} />

                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap', marginTop: '4px' }}>
                      {/* Map click mode toggle */}
                      <div style={{ display: 'flex', gap: '4px', background: dm('#f0f4f8', '#0f172a'), borderRadius: '40px', padding: '4px' }}>
                        <button
                          onClick={() => setMapClickMode('origin')}
                          style={{ padding: '6px 14px', borderRadius: '32px', border: 'none', background: mapClickMode === 'origin' ? '#2c5282' : 'transparent', color: mapClickMode === 'origin' ? 'white' : dm('#4a5568', '#94a3b8'), fontWeight: '600', fontSize: '11px', cursor: 'pointer', transition: 'all 0.2s' }}
                        >Set origin</button>
                        <button
                          onClick={() => setMapClickMode('destination')}
                          style={{ padding: '6px 14px', borderRadius: '32px', border: 'none', background: mapClickMode === 'destination' ? '#2c5282' : 'transparent', color: mapClickMode === 'destination' ? 'white' : dm('#4a5568', '#94a3b8'), fontWeight: '600', fontSize: '11px', cursor: 'pointer', transition: 'all 0.2s' }}
                        >Set destination</button>
                      </div>

                      {/* Travel mode */}
                      <div style={{ display: 'flex', gap: '6px', background: dm('#f0f4f8', '#0f172a'), borderRadius: '40px', padding: '4px' }}>
                        {modeOptions.map(({ key, icon, label }) => (
                          <motion.button
                            key={key} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                            onClick={() => setRouteMode(key)} title={label}
                            style={{ padding: '6px 12px', borderRadius: '32px', border: 'none', cursor: 'pointer', fontSize: '16px', transition: 'all 0.2s', background: routeMode === key ? '#2c5282' : 'transparent', color: routeMode === key ? 'white' : dm('#4a5568', '#94a3b8') }}
                          >{icon}</motion.button>
                        ))}
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <motion.button
                          ...
                          disabled={isRouting}
                          style={{ padding: '8px 20px', borderRadius: '30px', border: 'none', background: 'linear-gradient(135deg, #2c5282, #2b6cb0)', color: 'white', fontWeight: '700', fontSize: '13px', cursor: isRouting ? 'not-allowed' : 'pointer', opacity: isRouting ? 0.6 : 1, boxShadow: '0 4px 10px rgba(43,108,176,0.3)' }}
                        >{isRouting ? '⏳ Routing...' : 'Go →'}</motion.button>

                        {isRerouted && (
                          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={undoReroute}
                            style={{ padding: '8px 16px', borderRadius: '30px', border: 'none', background: '#ff9800', color: 'white', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}
                          >Undo</motion.button>
                        )}

                        {(origin || destination || routeCoords) && (
                          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={clearAll}
                            style={{ padding: '8px 12px', borderRadius: '30px', border: 'none', background: dm('#fed7d7', '#450a0a'), color: dm('#c53030', '#f87171'), fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}
                    ...
                    {routeError && (
                      <div style={{ background: dm('#fff5f5', '#450a0a'), padding: '10px 16px', borderRadius: '20px', fontSize: '12px', color: dm('#e53e3e', '#f87171'), border: `1px solid ${dm('#fed7d7', '#7f1d1d')}` }}>
                        ⚠️ {routeError}
                      </div>
                    )}
                    ...
                    {routeInfo && (
                      <motion.div
                        ...
                        style={{
                          background: dm('#ebf8ff', '#1e3a5f'),
                          padding: '12px 16px', borderRadius: '20px',
                          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                          flexWrap: 'wrap', gap: '12px',
                          border: `1px solid ${dm('#bee3f8', '#2c5282')}`,
                        }}
                      >
                        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                          <span style={{ fontSize: '13px', fontWeight: '700', color: dm('#2c5282', '#93c5fd'), display: 'flex', alignItems: 'center', gap: '6px' }}>📏 {routeInfo.distance} km</span>
                          <span style={{ fontSize: '13px', fontWeight: '700', color: dm('#2c5282', '#93c5fd'), display: 'flex', alignItems: 'center', gap: '6px' }}>⏱ {routeInfo.adjustedTime} min</span>
                          {routeInfo.trafficDelay > 0 && (
                            <span style={{ fontSize: '12px', color: '#d69e2e', display: 'flex', alignItems: 'center', gap: '4px' }}>🚦 +{routeInfo.trafficDelay} min delay</span>
                          )}
                        </div>
                        {trafficSummary && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <div style={{ width: '10px', height: '10px', borderRadius: '10px', background: trafficSummary.level === 'Heavy' ? '#c62828' : trafficSummary.level === 'Moderate' ? '#f57f17' : '#4caf50' }} />
                            <span style={{ fontSize: '11px', fontWeight: '600', color: dm('#4a5568', '#94a3b8') }}>Traffic: {trafficSummary.level}</span>
                          </div>
                        )}
                      </motion.div>
                    )}
          ...
          {!pickMode && waypointsList.length > 0 && showItineraryPanel && (
            <div style={{
              position: 'absolute', bottom: '80px', right: '10px', left: '10px',
              zIndex: 1002, pointerEvents: 'auto',
              background: dm('#EDE7F6', '#2d1b4e'),
              borderRadius: '18px', padding: '12px',
              boxShadow: '0 4px 18px rgba(0,0,0,0.13)',
              maxHeight: '40%', overflowY: 'auto',
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <div style={{ fontSize: '12px', fontWeight: '800', color: dm('#4A148C', '#ce93d8') }}>
                  🗺️ Itinerary • Stop {currentWaypointIndex + 1}/{waypointsList.length}
                </div>
                <button onClick={() => setShowItineraryPanel(false)} style={{ background: 'none', border: 'none', fontSize: '16px', cursor: 'pointer', color: dm('#666', '#94a3b8') }}>✕</button>
              </div>
              <div style={{ display: 'flex', gap: '4px', marginBottom: '8px', justifyContent: 'center' }}>
                {waypointsList.map((_, i) => (
                  <div key={i} style={{ width: i === currentWaypointIndex ? '24px' : '6px', height: '6px', borderRadius: '3px', background: i < currentWaypointIndex ? '#7B1FA2' : i === currentWaypointIndex ? '#AB47BC' : dm('#D1C4E9', '#4a2060'), transition: 'all 0.3s ease' }} />
                ))}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '150px', overflowY: 'auto' }}>
                {waypointsList.map((wp, i) => (
                  <div key={i} style={{
                    backgroundColor: i === currentWaypointIndex
                      ? dm('white', '#1e293b')
                      : i < currentWaypointIndex
                        ? dm('rgba(255,255,255,0.5)', 'rgba(255,255,255,0.1)')
                        : dm('rgba(255,255,255,0.3)', 'rgba(255,255,255,0.05)'),
                    borderRadius: '8px', padding: '6px 8px', fontSize: '10px',
                    fontWeight: i === currentWaypointIndex ? '700' : '400',
                    color: dm('#1A365D', '#cbd5e1'),
                  }}>
              ...
              {currentWaypointIndex < waypointsList.length - 1 && (
                <button onClick={goToNextWaypoint} style={{ marginTop: '8px', padding: '8px', background: 'linear-gradient(135deg, #7B1FA2, #AB47BC)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '12px', fontWeight: '700', width: '100%' }}>
                  Next Stop →
                </button>
              )}
            </div>
